@php
    $best_selling_products = Cache::remember('best_selling_products', 86400, function () {
        return filter_products(\App\Product::where('published', 1)->join('productconditions','productconditions.id','=','products.productcondition_id')->where('productconditions.name','new')->groupBy('model')->orderBy('num_of_sale', 'desc'))->limit(20)->get();
    });
