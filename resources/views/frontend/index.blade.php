...
<style>
    ...
    <!--category -->
    @php 
        $watch= \App\ProductSection::where('sec_name','all_watch')->first();
        $jewelry= \App\ProductSection::where('sec_name','all_jewelry')->first();
        $sell_re= \App\ProductSection::where('sec_name','sell_your_watch')->first();
        $service_re= \App\ProductSection::where('sec_name','service_request')->first();
    @endphp
       <div class="home_category_sec">
           <div class="container">
    <div class="row" style="margin-left:0 !important; margin-right:0 !important;">
       <div class="col-md-3 col-sm-6 subimg">
            <p style="text-align:center; font-size:17px;">
                <a href="{{ $watch->url }}" class="subimgtext">{{$watch->name}}</a>
            </p>
            <div class="feature-box">
                <a href="{{$watch->url}}" id="myBtn1"><img alt="Nature" src="{{ uploaded_asset($watch->img) }}" style="width: 100%;"></a>
            </div>
        </div>
        <div class="col-md-3 col-sm-6 subimg">
            <p style="text-align:center; font-size:17px;">
                <!--<a href="javascript:void(0)" id="myBtn1" onclick="doRequestWatch()" class="subimgtext"></a>-->
                  <!--<a href="{{ $jewelry->url }}" id="myBtn1" class="subimgtext"> {{$jewelry->name}}</a>-->
            ...
            <div class="feature-box">
                <a href="javascript:void(0)" id="myBtn1" onclick="doRequestWatch()" class="subimgtext" ><img alt="Nature" src="{{ uploaded_asset($jewelry->img) }}" style="width: 100%;"></a>
            </div>
        </div>
        <div class="col-md-3 col-sm-6 subimg">
            <p style="text-align:center; font-size:17px;">
                <a href="{{$sell_re->url}}" class="subimgtext">{{$sell_re->name}}</a>
            </p>
            <div class="feature-box">
                <a href="{{$sell_re->url}}"><img alt="Nature" src="{{ uploaded_asset($sell_re->img) }}"  style="width: 100%;"></a>
            </div>
        </div>
        <div class="col-md-3 col-sm-6 subimg">
            <p style="text-align:center; font-size:17px;">
                <a href="https://gcijewel.com/watch_and_jewelry_service/create"></a>
                <a href="{{$service_re->url}}" class="subimgtext"> {{$service_re->name}}</a>
            </p>
            <div class="feature-box">
                <a href="{{$service_re->url}}">
                    <img alt="Nature" src="{{ uploaded_asset($service_re->img) }}"  style="width: 100%;">
                </a>
            </div>
        </div>
    </div>
  ...
  {{-- Best Selling  --}}
    ...
    @if (get_setting('best_selling') == 1)
        @php
      $best_selling_products = (\App\Product::where('published',1)->where('featured', 1)->groupBy('model')->orderBy('num_of_sale', 'desc'))->limit(12)->get();
      @endphp
    ...
    <section class="mb-4">
        <div class="container">
            <div class="px-2 py-4 px-md-4 py-md-3 bg-white shadow-sm rounded">
                ...
                <div class="aiz-carousel gutters-10 half-outside-arrow" data-ride="slide" data-items="6" data-xl-items="5" data-lg-items="4"  data-md-items="3" data-sm-items="2" data-xs-items="2" data-arrows='true' data-infinite='true'>
    ...
    <!--</div>-->
     {{-- Watch Parts  --}}
    <!--<div id="section_watch_parts">-->

    <!--</div>-->
   {{-- Jewelry  --}}
    <div id="section_jewelry">

    </div>
    {{-- used condition  --}}
    <!--<div id="section_used_condition">-->

    <!--</div>   -->
  {{-- new condition  --}}
    <!--<div id="section_new_condition">-->
...
@section('script')
    <script>
        $(document).ready(function(){
            ...
            // });
            // $.post('{{ route('home.section.watch_parts') }}', {_token:'{{ csrf_token() }}'}, function(data){
            //     $('#section_watch_parts').html(data);
            //     AIZ.plugins.slickCarousel();
            // });
            $.post('{{ route('home.section.jewelry') }}', {_token:'{{ csrf_token() }}'}, function(data){
                $('#section_jewelry').html(data);
                AIZ.plugins.slickCarousel();
            });
            // $.post('{{ route('home.section.used_condition') }}', {_token:'{{ csrf_token() }}'}, function(data){
            //     $('#section_used_condition').html(data);
            //     AIZ.plugins.slickCarousel();
            // });
            //  $.post('{{ route('home.section.new_condition') }}', {_token:'{{ csrf_token() }}'}, function(data){
            //     $('#section_new_condition').html(data);
            //     AIZ.plugins.slickCarousel();
            // });
