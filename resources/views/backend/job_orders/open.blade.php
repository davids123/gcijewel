            ...
            <div class="mi_custome_table">
		        ...
		        <div class="card-body">
                          ...
                          @foreach($jobOrderData  as $key => $jobData)
                            ...
                            @endphp
                                ...
                                <tr>
                                    ...
                                    <td>  <a style="text-decoration:none;color:#000;" href="{{ route('job_orders.edit',['id'=>$jobData->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}"  title="{{ translate('Edit') }}">{{ date("m/d/20y", strtotime($jobData->created_at))}}</a></td>
                    ...
                    {{ $jobOrderData->appends(request()->input())->links() }}
                     @else
                        <p>
                          Showing {{$jobOrderData->count()}} of  {{$jobOrderData->count()}} entries
                        </p>
                    @endif
                    </div>
