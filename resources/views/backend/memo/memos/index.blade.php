...
<div class="row">
	<div class="col-md-12">
		<div class="card">
		    ...
		    <div class="card-body">
		        <table class="table aiz-table mb-0 memo_table_stock_modle_serial" id="memo_data">
		            <!-- <thead>
		            ...
		            <thead>
						...
						<th data-orderable="false"><input type="checkbox" class="select_count" id="select_count"  name="all[]"></th>
                          ...
                          <th><span class="missingtd">{{translate($Filter)}}</span></th>
		            ...
		            <tbody>
                        ...
                        @if(!empty($optData->due_date))
                            ...
                            @endif
                            @elseif($filterkey=='model_numbers')
                             @php $model=explode(',', $optData->model_numbers);  $model_number= implode(", " , $model); @endphp
                               <td  class="select-tr-edit" onclick="memoeditcallback()" data-href="{{route('memo.edit', ['id'=>$optData->id,  'lang'=>env('DEFAULT_LANGUAGE')] )}}" style="cursor: pointer;"> <span class="memo_span_custom">{{$model_number}} </span></td>
                            @elseif($filterkey=='stocks')
                               @php $stock=explode(',', $optData->stocks);  $stock_number= implode(", " , $stock); @endphp
                               <td  class="select-tr-edit" onclick="memoeditcallback()" data-href="{{route('memo.edit', ['id'=>$optData->id,  'lang'=>env('DEFAULT_LANGUAGE')] )}}" style="cursor: pointer;"><span class="memo_span_custom">{{$stock_number}}</span> </td>
                            
                            @elseif($filterkey=='sku')
                             @php $serial=explode(',', $optData->sku);  $serial_number= implode(", " , $serial); @endphp
                               <td  class="select-tr-edit" onclick="memoeditcallback()" data-href="{{route('memo.edit', ['id'=>$optData->id,  'lang'=>env('DEFAULT_LANGUAGE')] )}}" style="cursor: pointer;"> <span class="memo_span_custom">{{$serial_number}} </span></td>
                        	...
                        	@elseif($filterkey == 'company')
                        	@if($optData->customer_group=='reseller')
                        	 <td  class="select-tr-edit" onclick="memoeditcallback()" data-href="{{route('memo.edit', ['id'=>$optData->id,  'lang'=>env('DEFAULT_LANGUAGE')] )}}" style="cursor: pointer;">{{$optData->company}} </td>
                        	 @else
                        	 <td  class="select-tr-edit" onclick="memoeditcallback()" data-href="{{route('memo.edit', ['id'=>$optData->id,  'lang'=>env('DEFAULT_LANGUAGE')] )}}" style="cursor: pointer;">{{$optData->customer_name}} </td>
                        	 @endif
                            ...
                            <td  class="select-tr-edit mitash1" onclick="memoeditcallback()" data-href="{{route('memo.edit', ['id'=>$optData->id,  'lang'=>env('DEFAULT_LANGUAGE')] )}}" style="cursor: pointer;"><?php  echo money_format("%(#1n", $optData->sub_total)."\n";  ?></td>

                            @else

                            <td  class="select-tr-edit mitash2" onclick="memoeditcallback()" data-href="{{route('memo.edit', ['id'=>$optData->id,  'lang'=>env('DEFAULT_LANGUAGE')] )}}" style="cursor: pointer;"><?php  echo money_format("%(#1n", $optData->$filterkey)."\n";  ?></td>

                            @endif

                            @else

                            <td  class="select-tr-edit mitash3" onclick="memoeditcallback()" data-href="{{route('memo.edit', ['id'=>$optData->id,  'lang'=>env('DEFAULT_LANGUAGE')] )}}" style="cursor: pointer;"> {{$optData->$filterkey;}} </td>
		         ...
		         <div class="aiz-pagination">
                    ...
                    {{ $memoData->appends(request()->input())->links() }}
                     @else
                        <p>
                          Showing {{$memoData->count()}} of  {{$memoData->count()}} entries
                        </p>
                    @endif
                </div>
