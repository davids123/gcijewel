		...
		<div class="card">
		    ...
		    <div class="card-body">
				          ...
				          <tr>
	                          <td style="text-align:center;"><input type="checkbox" class="pro_checkbox" data-id="{{$invoice->memodetailsid}}" name="all_pro[]" value="{{$invoice->memodetailsid}}"></td>
								...
								@endif
                                  ...
                                  $model=explode(',', $invoice->model_numbers);  $model_number= implode(", " , $model); 
                                  
                                   $stock=explode(',', $invoice->stocks);  $stock_number= implode(", " , $stock); 
                                   
                                    $serial=explode(',', $invoice->skus);  $serial_number= implode(", " , $serial); 
								...
								<td>{{money_format("%(#1n", $invoice->order_total)."\n"}}</td>
		        ...
		        <div class="aiz-pagination">
                    ...
                    <p>
                        Showing {{ $memoInvoiceData->firstItem() }} to {{ $memoInvoiceData->lastItem() }} of  {{$memoInvoiceData->total()}} entries
                      </p>
                    {{ $memoInvoiceData->appends(request()->input())->links() }}
                     @else
                        <p>
                          Showing {{$memoInvoiceData->count()}} of  {{$memoInvoiceData->count()}} entries
                        </p>
                    @endif
            	</div>
		    </div>
