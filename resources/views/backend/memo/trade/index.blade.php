		    ...
		    <div class="card-body">
		                    ...
		                    <tr>
	                          ...
	                          <td style="text-align:center;"><input type="checkbox" class="pro_checkbox" data-id="{{$trade->id}}" name="all_pro[]" value="{{$trade->id}}"></td>
								...
								@php 
                                  ...
                                  $model=explode(',', $trade->model_numbers);  $model_number= implode(", " , $model); 
                                  
                                   $stock=explode(',', $trade->stocks);  $stock_number= implode(", " , $stock); 
                                   
                                    $serial=explode(',', $trade->skus);  $serial_number= implode(", " , $serial); 
		                        ...
		                        <td>{{  money_format("%(#1n", $trade->row_total)."\n"}}</td>
		        ...
		        <div class="aiz-pagination">
                   ...
                   @if($pagination_qty != "all")
                    <p>
                        Showing {{ $memotradeData->firstItem() }} to {{ $memotradeData->lastItem() }} of  {{$memotradeData->total()}} entries
                      </p>
                    {{ $memotradeData->appends(request()->input())->links() }}
                     @else
                        <p>
                          Showing {{$memotradeData->count()}} of  {{$memotradeData->count()}} entries
                        </p>
                    @endif
