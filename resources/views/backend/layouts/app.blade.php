...
<script src="https://code.jquery.com/ui/1.12.1/jquery-ui.js"></script>
    ...
    <script type="text/javascript">
...

		{
			...
			var apndHtml = '<tr class="selectes-product-append"><td style="width:10%;"><input type="checkbox" class="removeCheckedMemodata" value="'+apndProductId+'"></td><td><img src="'+apndProductimg+'" style="width:40px;height:50px;"></td><td>'+apndProducttxt+' <input type="hidden" name="memoitems['+apndProductId+'][price]" value="'+apndPrice+'"> <input type="hidden" name="memoitems['+apndProductId+'][stock]" value="'+apndProducttxt+'"></td><td class="desc_box">'+apndProductDesc+'</td>  <td>'+apndQty+'<input type="hidden" class="row_total_qty" name="memoitems['+apndProductId+'][quantity]" value="'+apndQty+'"></td> <td>Memo</td> <td>'+apndRowTotal+'<input type="hidden" class="row_total_memo" name="memoitems['+apndProductId+'][rowtotal]" value="'+apndRowTotal+'"></td></tr>';
 ...
 return x1 + x2;
	...
	$(".row_total_memo").each(function() {
		      ...
		      //console.log("remove comma ",rowCount);
				...
				// totalQty = parseFloat(rowCount) + parseFloat(totalQty);

			});
			
				$(".row_total_qty").each(function() {

				var rowCountqty = $(this).val();  
				rowCountqty = (rowCountqty.replace('$ ', ''));

				rowCountqty = parseFloat(rowCountqty.replace(/,/g, ''));
				totalQty = parseFloat(rowCountqty) + parseFloat(totalQty);
...
//               }
    ...
    // var memoTotalQTY = parseFloat(shippinfTxt) + parseFloat(totalQty);



				// totalQty =addCommas((parseFloat(totalQty).toFixed(2)).toString());//comma seperated with decomal string

    //     	memoTotalQTY =addCommas((parseFloat(memoTotalQTY).toFixed(2)).toString());//comma seperated with decomal string
							 ...
							 success: function(result) {
							     ...
							     alert(result.message);
							      location.reload(true);
							       window.location.href = "{{Route('memo.index')}}";
...
<script type="text/javascript" src="https://cdnjs.cloudflare.com/ajax/libs/jquery-cookie/1.4.1/jquery.cookie.min.js"></script>
	 <script>
			$("#barcode_label").on('click',function(){
			   ...
			   var allProID= $("#pagination_use_qty").val();
			   var availability = $("#availability").val();
			   var product_type = $("#product_type").val();
			   var warehouse_id = $("#warehouse_id").val();
				 var fltrBar = 'type:'+product_type+'|warehouse:'+warehouse_id+'|available:'+availability;
				 if(allProID == 'all'){
					 var barcodeArr = '{ "limit": "all", "filter":"'+fltrBar+'", "items": ""}';
					 console.log(barcodeArr);
				 }else{
					 var barcodeArr = '{ "limit": "custom", "filter":"", "items": '+proId+'}';
					 console.log(barcodeArr);
				 }
				 var date = new Date();
 				date.setTime(date.getTime() + (10 * 1000));
 				$.cookie('barcode',barcodeArr,{ expires: date });
			});
			$(document).ready(function ()	{
				var pageLoc = $(location).attr("pathname");
						pageLoc = pageLoc.split('/');
				var proId= $.cookie("barcode");
				if(pageLoc[4] == 'barcodestore'){
					if(proId != null){
					$.ajax({
    ...
    //     if($('#nav_status').is(":checked"))
    //     {
    //         $('#nav_status').val('truexcdsfds');
    //     }
    //   else if($('#nav_status').prop("checked") == false)
    //     {
    //         $('#nav_status').val('fgghg0');
    //     }
