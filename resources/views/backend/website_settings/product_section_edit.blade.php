	...
	<form class="p-4" action="{{ route('website.product_section_update', $pro->id) }}" method="POST" enctype="multipart/form-data">
		...
		<div class="card-body px-0">
		     <div class="form-group">
                ...
                <input type="text" placeholder="{{ translate('Name')}}"  name="name" class="form-control" value="{{ $pro->name }}" >
             </div>
              <div class="form-group">
                <label class="col-from-label" for="reference">
                    {{ translate('Url')}}
                </label>
                    <input type="text" placeholder="{{ translate('Url')}}"  name="url" class="form-control" value="{{ $pro->url }}" >
             </div>
