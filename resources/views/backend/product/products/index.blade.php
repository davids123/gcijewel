        ...
        <div class="row align-items-center product_index">
            ...
            @if($type != 'Seller')
                ...
                <div class="dropdown mb-2 mb-md-0">
                    ...
                    @if(Auth::user()->user_type == 'admin' ||  in_array('126', json_decode(Auth::user()->staff->role->inner_permissions)))
                    ...
                    @endif
                    @if(Auth::user()->user_type == 'admin' || in_array('127', json_decode(Auth::user()->staff->role->inner_permissions)))
                    ...
                    <a id="barcode_label" class="w-100 exportPro-class  print_barcode_btn " href="{{route('products.barcodestore')}}"   >Print Barcode/Label</a>
                </div>
            </div>
        @endif
    </div>
...
<div class="card">
    <form class="" id="sort_products" action="" method="GET">
        <div class="card-header row gutters-5 allproductsfltr">
            <div class="col">
            ...
            <div class="col-md-2 ml-auto d-flex">
                <select class="form-control form-control-sm aiz-selectpicker mb-2 mb-md-0" name="warehouse_id" id="warehouse_id" data-live-search="true">
                  ...
                  @foreach ($warehouseSelect as $whouse_filter)
            ...
            <div class="col-md-2 ml-auto d-flex">
                <select class="form-control form-control-sm aiz-selectpicker mb-2 mb-md-0" name="listing_type" id="product_type" data-live-search="true">
                    ...
                    @foreach ($SiteOptionSelect as $p_type_filter)
                          ...
                          @if($filterkey == 'custom')
                           ...
                           @elseif($filterkey == 'product_cost')
                               @if(Auth::user()->user_type == 'admin' || in_array('16', json_decode(Auth::user()->staff->role->inner_permissions)))
                                ...
                                @endif
                           @elseif($filterkey == 'partner')
                               @if(Auth::user()->user_type == 'admin' || in_array('18', json_decode(Auth::user()->staff->role->inner_permissions)))
                                  <th>{{translate($Filter)}}</th>
                                @endif
                        ...
                        @foreach($detailedProductList as $key => $detailedProduct)
                          ...
                          @foreach ($FilteredData as $filterkey => $Filter)
                              @if($filterkey == 'sku')
                               ...
                               @elseif($filterkey == 'dop')
                                 <td><a href="{{ route('products.viewproduct',['id'=>$detailedProduct->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}" style="color:#000;">{{date("m/d/20y", strtotime($detailedProduct->dop)) }}</a></td>
                              ...
                              @elseif($filterkey == 'thumbnail_img')
                                ...
                                <td><a href="{{ route('products.viewproduct',['id'=>$detailedProduct->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}" style="color:#000;">{{money_format("%(#1n", $detailedProduct->unit_price)."\n"}}</a></td>
                                 @elseif($filterkey == 'product_cost')
                                 @if(Auth
                                 ::user()->user_type == 'admin' ||  in_array('16', json_decode(Auth::user()->staff->role->inner_permissions)))
                                 ...
                                 @endif
                                  @elseif($filterkey == 'user_name')
                                    @if(Auth::user()->user_type == 'admin' ||  in_array('25', json_decode(Auth::user()->staff->role->inner_permissions)))
                                 <td><a href="{{ route('products.viewproduct',['id'=>$detailedProduct->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}" style="color:#000;"> {{$detailedProduct->companyname}}</a></td>
                                 @endif
                                 @elseif($filterkey == 'partner')
                                    @if(Auth::user()->user_type == 'admin' ||  in_array('18', json_decode(Auth::user()->staff->role->inner_permissions)))
                                 <td><a href="{{ route('products.viewproduct',['id'=>$detailedProduct->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}" style="color:#000;"> {{$detailedProduct->partner}}</a></td>
                                 @endif
                                 @elseif($filterkey == 'custom')
                                    @php
                                        $proType = isset($detailedProduct->productType) ? $detailedProduct->productType : '';
                                        $proTypeName = isset($proType->listing_type) ? $proType->listing_type : '';
                                    @endphp
                                    @if(is_array($Filter))
                                        @foreach ($Filter as $fkey => $Fval)
                                            @if(is_array($Fval))
                          ...
                          @endforeach
                          <td class="text-right">
                          @if(Auth::user()->user_type == 'admin' || in_array('123', json_decode(Auth::user()->staff->role->inner_permissions)))
                              ...
                              @if(Auth::user()->user_type == 'admin' || in_array('125', json_decode(Auth::user()->staff->role->inner_permissions)))
                          @if ($type == 'Seller')
   ...
   <script>
    ...
    <script type="text/javascript">
            ...
            // alert(proexpData);
  					...
  					if(proCheckID.length > 0)
  					{
  					    $('#barcode_label').removeClass("print_barcode_btn");
  					    $('#barcode_label').addClass('hoverProBtn');
  				    }
  				    else
  				    {
  					     $('#barcode_label').addClass("print_barcode_btn");
  				    }
                    if(proCheckID.length > 0){
