<style>
  ...
  @media print {
     ...
     .modal-body table.print-table tr th, .modal-body table.print-table tr td{
...
<div class="dropdown mb-2 mb-md-0 hideonprint trans_mi">
	...
	@if(Auth::user()->user_type == 'admin' || in_array('56', json_decode(Auth::user()->staff->role->inner_permissions)))
		...
		@endif
        @if(Auth::user()->user_type == 'admin' || in_array('140', json_decode(Auth::user()->staff->role->inner_permissions)))
			        ...
			        <table class="table aiz-table mb-0">
						...
						@foreach($transfer as $key => $optData)
							...
							<tr data-id="{{ $optData->id}}">
								...
								<td  class="RetData" data-target="#orderRetModal" data-toggle="modal" data-id="{{ $optData->id}}">{{$grand_total}}</td>
            ...
            <div class="aiz-pagination">
                 @if($pagination_qty != "all")
                    ...
                    {{ $transfer->appends(request()->input())->links() }}
                     @else
                        <p>
                          Showing {{$transfer->count()}} of  {{$transfer->count()}} entries
                        </p>
                    @endif
            </div>
