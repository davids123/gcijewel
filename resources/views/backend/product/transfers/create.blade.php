...
<div class="">
  ...
  <form class="p-4" id="saveTransferForm" method="POST">
    ...
    <div class="form-group row">
      ...
      <div class="form-group mb-0 text-right">
          ...
          <button type="button" id="TransBtn" class="btn btn-primary">{{translate('Save')}}</button>
    ...
    $( "#pro_transfer_search" ).autocomplete({
      open: function( event, ui ) {
        ...
        source: function(request, response) {
          ...
          if(stopMtplReq && stopMtplReq != 5){
            stopMtplReq.abort();
          }
          var srchExist = $("#pro_transfer_search").val();
              if(srchExist.length >= 4){
                var trValExist = "";
                $('.transautoco').each(function(){
                  trValExist = $(this).data("label");
                  if(srchExist == trValExist){
                    alert("Stock ID already exist");
                  }
                })
...
//  reverse(ui);
    //   console.log(ui.item);
      ...
      var pro_tble = "<tr class='returnItems'><td><input data-label="+ui.item.label+" type='hidden' class='transautoco' value="+ui.item.value+" />"+ui.item.model+"-"+ui.item.sku+"-"+ui.item.weight+"-"+ui.item.custom_6+"-("+ui.item.label+")</td><td class='rprocost' data-cost="+ui.item.product_cost+">"+formatter.format(ui.item.product_cost)+"</td><td><input class='form-control rproqty' name='proitemarr["+ui.item.value+"][]' type='text' value='1' readonly /></td><td class='rprocost2' data-subtotal="+ui.item.product_cost+">"+formatter.format(ui.item.product_cost)+"</td><td><button type='button' class='btn btn-soft-danger btn-icon btn-circle btn-sm protrash'><i class='las la-trash'></i></button></td></tr>";
        // reverse(pro_tble);
        var rowCount = $('#autocompletrans tr').length;
        if(rowCount > 0){
          $('#autocompletrans tr:first').before(pro_tble)
        }else{
...
// $(document).on("keyup",'#pro_transfer_search', function () {
//     var srchExist = $(this).val();
//     if(srchExist.length >= 4){
//       var trValExist = "";
//       $('.transautoco').each(function(){
//         trValExist += $(this).data("label");
...
$(document).on('click','#TransBtn',function(e) {
  ...
  $.ajax({
      ...
      success: function( response ) {
        ...
        title: 'Transfer has been added successfully',
