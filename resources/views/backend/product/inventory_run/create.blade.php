...
<div class="">
  <form class="p-4" id="saveInvRun" method="POST">
    @csrf
  ...
  <div class="form-group mb-0 text-left">
    <button type="button" id="InvRunBtn" class="btn btn-primary">{{translate('Submit')}}</button>
  </div>
</form>
</div>
@endsection
...
<script type="text/javascript">
  ...
  $( "#pro_stock_search" ).autocomplete({
    ...
    select: function (event, ui) {
      ...
      var rowCount = $('#autocompleInv tr').length;
      if(rowCount > 0){
        $('#autocompleInv tr:first').before(pro_tble)
      }else{
...
//   alert(searchVal);
  if(searchVal.length >= 5){
    ...
    var rowCount = $('#autocompleInv tr').length;
    if(rowCount > 0){
      $('#autocompleInv tr:first').before(searchData)
    }else{
      $('#autocompleInv').append(searchData);
    }
     return false;
...
// save data ajax
$(document).on('click','#InvRunBtn',function(e) {
  e.preventDefault();
  $.ajax({

      url: "{{route('inventory_run.save')}}",

      type: "POST",

      data: $('#saveInvRun').serialize(),
      ...
      success: function( response ) {
        ...
        title: 'Inventory has been added successfully',
      ...
      window.location.href = '{{ route("inventory_run.index") }}';
