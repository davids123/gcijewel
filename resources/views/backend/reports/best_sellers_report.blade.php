...
<div class="aiz-titlebar text-left mt-2 mb-3">
    ...
    <div class="dropdown-menu dropdown-menu-right">
                ...
                <form class="" action="{{route('best_seller_excel.index')}}" method="post">
                    @csrf
                ...
                @endphp --}}
                    <tbody>
                        ...
                        @foreach($best_seller_report as $key => $row)
                            ...
                            <tr>
                        <td><input type="checkbox" class="pro_checkbox" data-id="{{$row->id}}" name="all_pro[]" value="{{$row->model}}"></td>

                                <td>  @if($pagination_qty != "all") {{ ($best_seller_report->currentpage()-1) * $best_seller_report->perpage() + $key + 1 }} @else {{ $key + 1 }} @endif</td>
                                <td> {{$row->model}}</td>
                                <td> {{$row->totalQty}}</td>
