    ...
    public function collection(Collection $rows) {
        ...
        if($canImport) {
          ...
          foreach ($ProductType as $ProTypeval) {
            ...
            // echo $allProTypeArr[$ProTypeName]['id'] ."=".$ProTypeId."<br>";exit;
          ...
          // echo "<pre>";
           ...
           $ExistStock = array();
            ...
            if($category=='')
            {
                // echo $category."<br>"; 
                // echo $row['category'];exit;
                return flash(translate('category not valid !!'))->error();
                return false;
                break;
            }
            ...
            // exit;
               ...
               if(!empty($Sellers) && $row['supplier'] != ""){
                ...
                //   $seller[]=$SellerData->id;
                    // echo $Seller_name . "<br>".  $row['supplier']; exit;
                ...
                //  if(in_array($row['supplier'],$Seller_name))
                //  {
               ...
               if(!empty($brands) && $row['brand'] != ""){
                 ...
                 foreach ($brands as $brandsData) {
                   ...
                   $brands_name = $brandsData->name;
                    // echo $brands_name . "<br>".$row['brand'];exit;
               ...
               if(!empty($warehouse) && $row['warehouse'] != ""){
                 ...
                 foreach ($warehouse as $warehouseData) {
                   ...
                   $warehouse_name = $warehouseData->name;
                        // echo $warehouse_name. "<br>".$row['warehouse']; exit;
               ...
               if($Productconditions_id  < 1 && $importConVal != ""){
                ...
                $today = date("H:i:s");
                $dopDate= $date ." " . $today;
                // echo $row['sale_price_request']; exit;
                $proCreateArr = array(
                  ...
                  //   'sale_price_request'=>$row['sale_price_request'],
                ...
                // $proActsale_prise_request=$proCreateArr['sale_price_request'];
                // dd($proCreateArr['sale_price_request']); exit;
                if($tags != ''){
                  $proCreateArr['tags'] = $tags;
                }
                ...
                if ($stocks != "") {
                 ...
                 $prodId = isset($ProductArr[$stocks]) ? $ProductArr[$stocks] : "";
                  ...
                  if ($prodId > 0) {
                    ...
                    // if($proCreateArr['sale_price_request'] != "")
                    // {
                    //   $proActsale_prise_request= $proCreateArr['sale_price_request'];
                    // }
                    // else
                    // {
                    //     $proActsale_prise_request = $Actproduct->sale_price_request;
                    // }
                    // echo "dgfdghf";
                    // echo $proActsale_prise_request; exit;
                        // dd(gettype($RProCArr));exit;
                    foreach ($RProCArr as $RProCskey => $RProCsval) {
                        // echo "xcxv";exit;
                      // code...
                      $RPcustomVal = isset($row[$RProCskey]) ? $row[$RProCskey] : '';
                      $prod=(string)$Actproduct;
                    //   dd($Actproduct->$prod);
                      $ActcustomVal = isset($Actproduct->$prod) ? $Actproduct->$prod : '';
                    ...
                    // echo $Productconditions_id; exit;
                  if(!empty($Productconditions_id)){
                  ...
                  if(!empty($product_name)){
                    ...
                    if(!empty($row['msrp'])){
                      ...
                      // dd($proCreateArr); exit;
                      // echo "Dgfdgfdgfdh"; exit;
