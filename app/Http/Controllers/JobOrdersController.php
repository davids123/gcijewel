   ...
   // Warehouse Start
...

    {
      ...
      foreach ($job_detail_id as $jo_key => $jo_d_id) {
        ...
        $newlisting_type=$Request->listing_type[$jo_key];
        $new_stockId=$Request->stock_id[$jo_key];
        $newModel=$Request->model_number[$jo_key];
        $newSerialN=$Request->serial_number[$jo_key];
        $newWeight=$Request->weight[$jo_key];
        $newScrewCount=$Request->screw_count[$jo_key];
        $newdateF=$Request->date_of_return[$jo_key];
        $newagent=$Request->agent[$jo_key];
        $newestimated_cost=$Request->estimated_cost[$jo_key];
        // $new_date_of_r=$Request->estimated_date_return[$jo_key];
        ...
        $jo_order_detail = JobOrderDetail::select('job_order_details.*','agents.first_name')
            ...
            $array['userIp'] = $Request->ip();
            $array['user']  = $user->id;
            $array['olddata']  =$jo_order_detail;
            $array['new_bag_number']= $new_bag_number;
            $array['new_listing_type']=$newlisting_type;
            $array['new_stock_id']=$new_stockId;
            $array['new_model_n']=$newModel;
            $array['new_en_code']=$newestimated_cost;
            $array['new_Date_f']=$newdateF;
            $array['new_serial_n']=$newSerialN;
            $array['new_weight']=$newWeight;
            $array['new_agent']=$newagent;
            $array['new_screw_count']=$newScrewCount;
            $array['stock_id'] = 'JO000'.$id;
            $array['from'] = env('MAIL_FROM_ADDRESS');
            $array['AlljoeditTag'] = $AlljoeditTag;
            $array['jo_order'] = $jo_order;
            $array['allcountry'] = $allcountry;
            $array['jo_order_detail'] = $jo_order_detail;
            $array['from'] = env('MAIL_FROM_ADDRESS');
            Mail::send('frontend.backendMailer.jo_edit_email', $array, function($message) use ($array) {
                $message->to(env("StockManager"));
                $message->subject('User Activity Notification');
            });
    ...
    //         $array['userIp'] = $Request->ip();
    //         $array['user']  = $user->id;
    //         $array['oldcompny']  = $Request->company_name;
    //         $array['stock_id'] = 'JO000'.$id;
    //         $array['from'] = env('MAIL_FROM_ADDRESS');
    //         $array['AlljoeditTag'] = $AlljoeditTag;
    //         $array['jo_order'] = $jo_order;
