...
class HomeController extends Controller
{
    ...
    {
        return view('frontend.pdfloader');
    }
    ...
    * Show the customer/seller dashboard.
    ...
    public function load_watch_parts_section()
    {
        return view('frontend.partials.watch_parts_section');
    }
    public function load_jewelry_section()
    {
        return view('frontend.partials.jewelry_section');
    }
    public function load_used_product_section()
    {
        return view('frontend.partials.used_condition_section');
    }
    public function load_new_product_section()
    {
        return view('frontend.partials.new_condition_section');
    }
    ...
    {
        $detailedProduct  = Product::with('reviews', 'brand', 'stocks', 'user', 'user.shop','category','productcondition','productType')->select('products.*','productconditions.name as productconditions_name','product_types.listing_type','warehouse.name as warehouse_name')
        ...
        if($detailedProduct != null)
        {
            ...
            $memo = MemoDetail::select('memo_details.*','memos.memo_number', 'memos.customer_name','memo_details.item_status','retail_resellers.customer_name','retail_resellers.customer_group','retail_resellers.company')
                     ...
                     ->where('memo_details.product_id',$product_id)
                     ->first();  
            $JobOrderstatus = JobOrder::select('job_orders.job_status')
                     ...
                     ->where('job_order_details.jo_product_id',$product_id)
                     ->where('job_orders.job_status',2)
                     ->first();
            // $products  =  Cache::remember('best_selling_products', 86400, function () {
            //                 return filter_products(\App\Product::where('published', 1)->groupBy('model')->orderBy('num_of_sale', 'desc'))->limit(6)->get();
            //             });
        }
        else
        {
            ...
            $JobOrderstatus = JobOrder::select('job_orders.job_status')
                                 ...
                                 ->where('job_orders.job_status',2)
                                 ->first();
            // $products  =  Cache::remember('best_selling_products', function () {
            //                 return filter_products(\App\Product::where('published', 1)
            //                 ->groupBy('model')->orderBy('num_of_sale', 'desc'))->skip(0)->take(6)->get();
            //             });
                    }
                    $arry=array('2',4,6,3,5);
                $products=Product::inRandomOrder()
                            ...
                            ->whereIn('memo_details.item_status',$arry)
                            ->select('products.*',DB::raw('SUM(product_stocks.qty) as toal_qty'),'brands.name as brand_name', 'categories.name as category_name')
                            ->orderBy('toal_qty','DESC')
                            ->limit(6)->get();
            // $products= Product::where('published', 1)
            //                 ->groupBy('model')->orderBy('num_of_sale', 'desc')->limit(6)->get();
                    // dd($products);
            

      
        // Product::select('products.*', 'brands.name as brand_name', 'categories.name as category_name')
        // ->join('brands','brands.id','=','products.brand_id')
        // ->join('categories','categories.id','=','products.category_id')
        ...
        if($detailedProduct != null && $detailedProduct->published){
            ...
            if($detailedProduct->digital == 1){
               return view('frontend.digital_product_details', compact('detailedProduct'));
                ...
                $pro_metal=$detailedProduct->metal;
                // dd($detailedProduct);
                $stock_id = $detailedProduct->stock_id;
                $listning_type= Producttype::where('id','=',$detailedProduct->product_type_id)->first()->listing_type;
                $prodQtyArr =Product::with('reviews','stocks')
                                    ->join('product_stocks','products.id','=','product_stocks.product_id')
                                    ->where('products.published', '=', '1')
                                    ->where('product_stocks.qty','>',0)
                                    ->where('products.model', '=', $modelNumber)
                                    ->where('products.metal',$pro_metal)
                                    ->orderBy('products.id', 'DESC')
                                    // ->groupBy('products.metal')
                                    ->select(DB::raw('SUM(product_stocks.qty) as prostock'));
              
        $watch_parts=array('Watch Parts','Bands','Band','Dial','Links','Bezel','Crystal','Insert');
        if(in_array($detailedProduct->listing_type,$watch_parts))
        {
            $prodQtyArr=$prodQtyArr->groupBy('products.metal');
        }
        else
        {
           $prodQtyArr=$prodQtyArr->groupBy('products.model') ;
        }
        $prodQtyArr=$prodQtyArr->first();
            //     Product::select(DB::raw('SUM(product_stocks.qty) as prostock'))
            //     ->join('product_stocks','products.id','=','product_stocks.product_id')
            //   ->where('products.published', '=', '1')
            //     ->where('products.model', '=', $modelNumber)
            //      ->groupBy('products.model','products.metal')
            //      ->first();
                //  dd($prodQtyArr);
                $prostock = isset($prodQtyArr->prostock) ? $prodQtyArr->prostock : '';
                // dd($prostock);
                $linkpage = 'https://gciwatch/'.$listning_type.'/'.$modelNumber.'/'.$stock_id;
                return view('frontend.product_details', compact('detailedProduct','linkpage','products', 'prostock','memo','JobOrderstatus'));
            }
        }
        abort(404);
    }
...
//        }
    ...
    public function captch_service(Request $request)
    {
         return view('frontend.partials.captcha_service');
    }
}
