...
class InventoryrunController extends Controller
{
   // Return Start
      ...
      //  ->get();
       ...
       if ($request->search != null){
        ...
        if( $request->pagination_qty == "all")
         ...
         {
            $InventoryRun = $PurchasesProduct->paginate($pagination_qty);
         }
    

        return view("backend.product.inventory_run.index", compact('InventoryRun','sort_search','pagination_qty'));
...

    {
        ...
        $ary_status=array('1','0','2','4','6');
        $proTypeLS = ProductType::where('listing_type',$Request->listing_type)
                                // ->leftJoin('products','products.product_type_id','=','product_types.id')
                                // ->leftJoin('memo_details','memo_details.product_id','=','products.id')
                                // ->whereNotIn('memo_details.item_status',$ary_status)
                                ->get();
                                // print_r($proTypeLS);exit;
   ...
   $InventoryData = InventoryRun::select('inventory_runs.id','inventory_runs.listing_type','inventory_runs.missing','inventory_runs.duplicate','inventory_runs.extra','inventory_runs.extrakeyup','inventory_runs.created_at','product_types.product_type_name','users.name','warehouse.name as warehouse_name','warehouse.address')
    ...
    // echo $Request->listing_type;exit;
     $date=date('m/d/20y',strtotime($InventoryData->created_at));
        ...
        $array['date']=$date;
        $array['stockId'] = $missingstr;
        $array['extra']=$allExtra;
        $array['missing']=$missingStock;
        $array['duplicate']=$duplicateStock;
...

{
...

  {
    ...
    $InventoryData = InventoryRun::select('inventory_runs.id','inventory_runs.stock','inventory_runs.listing_type','inventory_runs.missing','inventory_runs.duplicate','inventory_runs.extra','inventory_runs.extrakeyup','inventory_runs.created_at','product_types.product_type_name','users.name','warehouse.name as warehouse_name','warehouse.address')
    ->leftJoin('users','users.id','=','inventory_runs.user')
    ->leftJoin('product_types','product_types.id','=','inventory_runs.listing_type')
    ->leftJoin('warehouse','warehouse.id','=','inventory_runs.warehouse_id')
    ->orderBy('inventory_runs.id', 'desc')
    ...
    // echo gettype($duplicateStock);exit;
    $duplicateStock=explode(',',$duplicateStock);
    $duplicateStock=implode(' , ', $duplicateStock);
    $missingStock=explode(',',$missingStock);
    $missingStock=implode(' , ', $missingStock);
    // echo $duplicateStock; exit;
    $stock=explode(',',$stock);
    $stock=implode(' , ', $stock);
    // echo implode(' , ', $stock); exit;
    $comma = "";
     if(!empty($optData->extra)){
       $comma = ",";
     }
