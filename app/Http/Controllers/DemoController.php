    ...
    class DemoController extends Controller
    {
         ...
         public function BandsExcel()
        {
            ...
            $product =Product::with('reviews', 'brand', 'stocks', 'user', 'user.shop','category','productcondition','productType')
                            ...
                            ->leftJoin('memos', 'memos.id', '=', 'memo_details.memo_id')
                            ->LeftJoin('retail_resellers', 'memos.customer_name', '=', 'retail_resellers.id')
                            ->where('product_types.listing_type','Bands')
                            ->get();
                        // dd($low_stock);
                         if(!blank($product))
                        {
        ...
        public function JewelriesExcel()
        {
            ...
            $product =Product::with('reviews', 'brand', 'stocks', 'user', 'user.shop','category','productcondition','productType')
                            ...
                            ->leftJoin('memos', 'memos.id', '=', 'memo_details.memo_id')
                            ->LeftJoin('retail_resellers', 'memos.customer_name', '=', 'retail_resellers.id')
                            ->where('product_types.listing_type','Jewelry')
                            ->get();
                        // dd($low_stock);
                         if(!blank($product))
                        {
        ...
        public function BangleExcel()
        {
            ...
            $product =Product::with('reviews', 'brand', 'stocks', 'user', 'user.shop','category','productcondition','productType')
                            ...
                            ->leftJoin('memos', 'memos.id', '=', 'memo_details.memo_id')
                            ->LeftJoin('retail_resellers', 'memos.customer_name', '=', 'retail_resellers.id')
                            ->where('product_types.listing_type','Bangle')
                            ->get();
                        if(!blank($product))
                        {
                            header('Content-Type: text/csv; charset=utf-8');
         ...
         public function watcparts()
        {
          $date = new \DateTime(null, new \DateTimeZone('America/Los_Angeles'));
            $pst = $date->format('m-d-Y H:i');
            $watchp=array('Band','Bezel','Dial','Watch Parts');
            $product =Product::with('reviews', 'brand', 'stocks', 'user', 'user.shop','category','productcondition','productType')
                            ->leftJoin('warehouse','warehouse.id','=','products.warehouse_id')
                            ->leftJoin('users','users.id','=','products.supplier_id')
                            ->leftJoin('product_types','product_types.id','=','products.product_type_id')
                            ...
                            ->leftJoin('memos', 'memos.id', '=', 'memo_details.memo_id')
                            ->LeftJoin('retail_resellers', 'memos.customer_name', '=', 'retail_resellers.id')
                            ->whereIn('product_types.listing_type',$watchp)
                            ->get();
                        if(!blank($product))
                        {
                            header('Content-Type: text/csv; charset=utf-8');
         ...
         public function allListingExcel()
        {
            $product=array();
            $listing_type=  \App\Models\Producttype::select('product_types.*')->groupBy('listing_type')->get();
            // dd($listing_type);
            foreach($listing_type as $row)
            {
                $date = new \DateTime(null, new \DateTimeZone('America/Los_Angeles'));
                $pst = $date->format('m-d-Y H:i');
                $product[] =Product::with('reviews', 'brand', 'stocks', 'user', 'user.shop','category','productcondition','productType')
                            ...
                            ->leftJoin('memos', 'memos.id', '=', 'memo_details.memo_id')
                            ->LeftJoin('retail_resellers', 'memos.customer_name', '=', 'retail_resellers.id')
                            ->where('product_types.listing_type',$row->listing_type)
                            ->get();
            }
                            // dd(!blank($product));
                            // exit;
        ...
        {
            $today = date("H");
            // mail("samd@mitashdigital.com","My Old Test",$today);
            // dd($daily_mail);
          if($today == '08' || $today == '01' || $today == '13'){
    
            // mail("samd@mitashdigital.com","My Test",$today);
            ...
            Mail::send('frontend.allmails.dr_mail_file', $array, function($message) use ($array,$pdf) {
                ...
                $message->attachData($pdf->output(), "end_of_day_summary.pdf");
                 if(!empty($this->watchExcel()))
                    {
                        $message->attachData($this->watchExcel(), "watch.csv");
                    }
                     if(!empty($this->StrapsExcel()))
                    {
                        $message->attachData($this->StrapsExcel(), "straps.csv");
                    }
                    if(!empty($this->BandsExcel()))
                    {
                        $message->attachData($this->BandsExcel(), "bands.csv");
                    }
                    if(!empty($this->JewelriesExcel()))
                    {
                        $message->attachData($this->JewelriesExcel(), "Jewelry.csv");
                    }
                    if(!empty($this->BezelExcel()))
                    {
                        $message->attachData($this->BezelExcel(), "Bezel.csv");
                    }
                    if(!empty($this->BraceletExcel()))
                    {
                        $message->attachData($this->BraceletExcel(), "Bracelet.csv");
                    }
                    if(!empty($this->RingExcel()))
                    {
                        $message->attachData($this->RingExcel(), "Ring.csv");
                    }
                    if(!empty($this->NecklaceExcel()))
                    {
                        $message->attachData($this->NecklaceExcel(), "Necklace.csv");
                    }
                    if(!empty($this->EarringExcel()))
                    {
                        $message->attachData($this->EarringExcel(), "Earring.csv");
                    }
                    if($this->DialExcel()!==null)
                    {
                        $message->attachData($this->DialExcel(), "dial.csv");
                    }
                    if($this->BangleExcel()!==null)
                    {
                        $message->attachData($this->BangleExcel(), "Bangle.csv");
                    }
                    if($this->watcparts()!==null)
                    {
                        $message->attachData($this->watcparts(), "watch parts.csv");
                    }
                // if($this->allListingExcel()!== null)
                // {
                //       $message->attachData($this->allListingExcel(), "watchParts.csv");
                // }
