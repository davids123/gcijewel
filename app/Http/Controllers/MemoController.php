...
class MemoController extends Controller
{
  ...
  $pagination_qty = isset($request->pagination_qty)?$request->pagination_qty:25;
      ...
      $search_data = $request->search;
        ...
        $arrSt = array(0,1);
        $memoQuery = Memo::select('memos.*', 'retail_resellers.company','retail_resellers.customer_name as rcustomername','memo_payments.payment_name','retail_resellers.customer_group','memo_details.item_status')
                  ...
                  ->selectRaw('GROUP_CONCAT(sku) as sku')
    ...
    public function closememo(Request $request)
    {
     ...
     $pagination_qty = isset($request->pagination_qty)?$request->pagination_qty:25;
      ...
      $closeQry =
      Memo::select('memos.*', 'retail_resellers.company','retail_resellers.customer_name as rcustomername','memo_payments.payment_name','retail_resellers.customer_group','memo_details.item_status')
      ->join('retail_resellers', 'memos.customer_name', '=', 'retail_resellers.id')
      ->join('memo_details', 'memos.id', '=', 'memo_details.memo_id')
      ->join('products', 'memo_details.product_id', '=', 'products.id')
      ->join('product_stocks','products.id','=','product_stocks.product_id')
...
public function floatvalue($val){
        ...
        //   $post->item_status = 1;
          ...
          if($cs_name != ""){
            ...
            $curr_name = $user->name;
              ...
              $proLog->prodcut_id = $pId;
              if($csName->customer_group == 'reseller'){
                $proLog->activity = addslashes('STOCK ID <a href="#">'.$proDetailStock.'</a> was Memo To Memo <a href="#" class="memourl" data-uid="'.$memoID.'">'.$memon.'</a> By Customer <a href="#" class="csurl" data-uid="'.$cs_name.'">'.$csName->company.'</a> by '.$curr_name.' on');
    ...
    public function memoedit(Request $request, $id)
    {
        ...
        $memo_details_product = DB::table('memo_details')
            ...
            $product_qty=DB::table('memo_details')
                              ->select('memo_details.*' ,DB::raw('SUM(memo_details.product_qty) as totalQty'),DB::raw('SUM(memo_details.row_total) as sub_total'),'products.stock_id','productconditions.name','products.photos','products.description','products.cost_code','products.product_cost','products.custom_6','products.custom_3','products.custom_4','products.custom_5','product_types.listing_type','products.model','products.weight','products.paper_cart','products.metal','product_stocks.sku')
                              ->join('products', 'memo_details.product_id', '=', 'products.id')
                              ->join('product_types', 'products.product_type_id', '=', 'product_types.id')
                              ->leftJoin('productconditions','products.productcondition_id','=','productconditions.id')
                              ->join('product_stocks','products.id','=','product_stocks.product_id')
        ...
        // get deposit payment 
        $totalqty="";
        foreach($product_qty as $memo_d)
        {
            $totalqty=$memo_d->totalQty;
            // dd($totalqty);
        }
        // dd($totalqty);
        ...
        $sub_total = '$ '.number_format($memo_d->sub_total, 2);
        $sale_tax=$memo->sale_tax;
        $shipping_charges = '$ '.number_format( $memo->shipping_charges, 2);

        $remaining_total = $memo_d->sub_total - $totalPaidAmount +$memo->shipping_charges+$sale_tax;
        ...
        return view('backend.memo.memos.edit', compact('memo','totalqty',/*'activitylogusr','activitylogStatus','activitylogData',*/'totalPaidAmount','memoNumbermodal','remaining_total','memo_details_product','sub_total','sale_tax','shipping_charges'));
    }
    ...
    public function memoUpdate(Request $request, $id)
    {
      ...
      $memo_details_id=$memoUpdate->id;
        $rowtoal=str_replace( array( '\'', '"',
      ',' , ';', '<', '$' ), ' ', $request->row_total);
      $rowtoal =preg_replace('/(?<=\d)\s+(?=\d)/', '', $rowtoal);
    //   dd($rowtoal);
        $memo_details=MemoDetail::where('memo_id',$memo_details_id)->first();
        // echo $request->row_total . "<br>".$memo_details->row_total;exit;
         if(!empty($rowtoal)){
          if($rowtoal != $memo_details->row_total){
            $old_name = $memo_details->row_total;
            $proLog = new Activitylog();
            $proLog->type = 'memo';
            $proLog->user_id = $curr_uid;
            $proLog->prodcut_id = $memo_details->product_id;
            $proLog->activity = addslashes('STOCK ID '.$request->stock_id.' Such as Price was changed from '.$old_name.' to '.$rowtoal.' by '.$curr_name.' on');
            $proLog->action = 'edit';
            $proLog->save();
          }
        }

        $memo_details->row_total=$rowtoal;
     ...
     // echo  $memoUpdate->sale_tax;exit;
      ...
      $memoUpdate->sub_total=$rowtoal;
      
       //dd( $memoUpdate->sub_total);
    ...
    {
       $act=  Memo::select('memos.id','memo_details.id as memo_dId','products.id as pro_id')->join('memo_details','memo_details.memo_id','memos.id')->join('products','products.id','memo_details.product_id')->where('memos.id',$id)->first();
       $proId=$act->pro_id;
       $prostoc=ProductStock::where('product_id',$proId)->first();
       $prostoc->qty=$prostoc->qty+1;
       $prostoc->save();
       $updPo=Product::findOrFail($proId);
       $updPo->published='1';
       $updPo->save();
       $act_d=Activitylog::where('activitylogs.prodcut_id',$proId)->delete();
        // dd($act_d);
    ...
    //   echo $typedata;
      $memoProductData = MemoDetail::selectRaw('GROUP_CONCAT(product_id) as p_id')
                        ...
                        // ->leftJoin('product_stocks','product_stocks.product_id','memo_details.product_id')
                        ->where('memo_details.product_qty','<',0)
                        // ->where('products.published','!=',1)
                        // ->whereIn('memo_details.item_status',$memoItemStatus)
                        ->get();
      // $memoProductData = DB::table('memo_details')
      //                   ->selectRaw('GROUP_CONCAT(product_id) as p_id')
      //                   ->whereIn('memo_details.item_status',$memoItemStatus)
      //                   ->groupBy('memo_details.product_id')
      //                   ->toSql();
      //                   dd($memoProductData);
                    $memoP_id = '';
                    // print_r($memoProductData)."<pre>";exit;
                    ...
                    if($memoP_id == ''){
                        $allProductData = Product::select('products.*','product_stocks.qty','product_stocks.sku')
                                        ...
                                        //     ->where('products.published','=',1)
                                            // ->whereNotIn('memo_details.item_status',$memoItemStatus)
      ...
      foreach ($allProductData as $productData) {
        ...
        //   dd($ProductTypeID->listing_type);
            ...
            else if($ProductTypeID->listing_type == 'Bangle')
            {
             ...
             else if($ProductTypeID->listing_type == 'Jewelry')
            {
                $reqDescr = $ProductConditionValue.' : '.$model.'-'.$serialNumber.'-'.$weight.'g -'.$metal.'-'.$paperCart.'';
            }
            ...
            {
                $reqDescr = $ProductConditionValue.' : '.$model.'-'.$metal.')-'.$weight.'g ';
            }
            else{
              $reqDescr = $ProductConditionValue.' : '.$model.'-'.$metal.')';
            }
    ...
    //  echo $memoactiondata; exit;
      ...
      foreach ($memoproItems as $proItems) {
        ...
        $memoDetails->item_status =2 ;
            if($csName->customer_group=="retail")
            {
                ...
                $updateMail= 'STOCK ID '.$stock_id.'  was Invoice Sale '. $memon.' by Customer'. $csName->customer_name.'  by '.$curr_name.' on';
            }
            else
            {
            ...
            $updateMail= 'STOCK ID '.$stock_id.'  was Invoice Sale '. $memon.' by Customer'. $csName->company.'  by '.$curr_name.' on';
            }
        ...
        if($csName->customer_group=="retail")
        {
            ...
            $updateMail= 'STOCK ID '.$stock_id.'  was Return From Memo  '. $memon.' by Customer'. $csName->customer_name.'  by '.$curr_name.' on';
        }
        else
        {
          $proLog->activity = addslashes('STOCK ID  <a href="#" class="prourl" data-uid="'.$proId.'">'.$stock_id.'</a> was Return From Memo <a href="#" class="memourl" data-uid="'.$memoDetails->memo_id.'">'. $memon.' </a> by Customer <a href="#" class="csurl" data-uid="'.$cs_name.'">'. $csName->company.' </a>  by '.$curr_name.' on');
            ...
            $updateMail= 'STOCK ID '.$stock_id.'  was Return From Memo '. $memon.' by Customer'. $csName->company.'  by '.$curr_name.' on';
        }
        ...
        if($csName->customer_group=="retail")
        {
            $proLog->activity = addslashes('STOCK ID  <a href="#" class="prourl" data-uid="'.$proId.'">'.$stock_id.'</a> was Trade TRD <a href="#" class="memourl" data-uid="'.$memoDetails->memo_id.'">'. $memon.' </a> by Customer <a href="#" class="csurl" data-uid="'.$cs_name.'">'. $csName->customer_name.' </a>  by '.$curr_name.' on');
               ...
               $updateMail= 'STOCK ID '.$stock_id.'  was Trade TRD  '. $memon.' by Customer'. $csName->customer_name.'  by '.$curr_name.' on';
        }
        else
        {
          $proLog->activity = addslashes('STOCK ID  <a href="#" class="prourl" data-uid="'.$proId.'">'.$stock_id.'</a> was Trade TRD <a href="#" class="memourl" data-uid="'.$memoDetails->memo_id.'">'. $memon.' </a> by Customer <a href="#" class="csurl" data-uid="'.$cs_name.'">'. $csName->company.' </a>  by '.$curr_name.' on');
               ...
               $updateMail= 'STOCK ID '.$stock_id.'  was Trade TRD '. $memon.' by Customer'. $csName->company.'  by '.$curr_name.' on';
        }
        ...
        if($csName->customer_group=="retail")
        {
            $proLog->activity = addslashes('STOCK ID  <a href="#" class="prourl" data-uid="'.$proId.'">'.$stock_id.'</a> was  Voided From Memo <a href="#" class="memourl" data-uid="'.$memoDetails->memo_id.'">'. $memon.' </a> by Customer <a href="#" class="csurl" data-uid="'.$cs_name.'">'. $csName->customer_name.' </a>  by '.$curr_name.' on');
              ...
              $updateMail= 'STOCK ID '.$stock_id.'  was  Voided From Memo  '. $memon.' by Customer'. $csName->customer_name.'  by '.$curr_name.' on';
        }
        else
        {
          ...
          $updateMail= 'STOCK ID '.$stock_id.'  was  Voided From Memo  '. $memon.' by Customer'. $csName->company.'  by '.$curr_name.' on';
        }
        ...
        if($csName->customer_group=="retail")
        {
            $proLog->activity = addslashes('STOCK ID  <a href="#" class="prourl" data-uid="'.$proId.'">'.$stock_id.'</a> was Trade NGD TNGD <a href="#" class="memourl" data-uid="'.$memoDetails->memo_id.'">'. $memon.' </a> by Customer <a href="#" class="csurl" data-uid="'.$cs_name.'">'. $csName->customer_name.' </a>  by '.$curr_name.' on');
             ...
             $updateMail= 'STOCK ID '.$stock_id.' was Trade NGD TNGD  '. $memon.' by Customer'. $csName->customer_name.'  by '.$curr_name.' on';
        }
        else
        {
         ...
         $updateMail= 'STOCK ID '.$stock_id.' was Trade NGD TNGD  '. $memon.' by Customer'. $csName->company.'  by '.$curr_name.' on';
        }
      ...
      $proLogNew->save();
        ...
        // $updateMail= 'A Memo '.$memon.'was closed by user'.$curr_name.' on';
        $proLogusr->action = 'closedByUser';
        $proLogusr->save();
        // $memo = Memo::findOrFail($memoDetails->memo_id);
        $array['csName'] = $csName;
        $array['LsProAct'] = $updateMail;
    ...
    public function invoice(Request $request)
    {
       ...
       $invoiceQry = Memo::select('memos.id as memoid','memo_details.id as memodetailsid','memo_details.item_status','retail_resellers.*',
                              ...
                              ->orderBy('memo_details.memo_id', 'DESC')
                              ->groupBy('memo_details.memo_id');
        ...
        if($sort_search != null){
         ...
         {
            $memoInvoiceData = $invoiceQry->paginate($pagination_qty);
         }
        // $pagination  = $invoiceQry->paginate($pagination_qty);
        // $memoInvoiceData=$invoiceQry->get();
        // dd($memoInvoiceData);

        return view("backend.memo.invoice.index", compact('memoInvoiceData','pagination_qty','sort_search'));
    }
    ...
    public function trade(Request $request)
    {
       ...
       $tradeQry = Memo::select('memos.order_total','memos.id as memoid','memo_details.id','memo_details.item_status','retail_resellers.customer_name','products.stock_id','products.model','memo_details.row_total','memo_details.status_updated_date','product_stocks.sku')
        ...
        if($sort_search != null){
         ...
         {
            $memotradeData = $tradeQry->paginate($pagination_qty);
         }
        // $pagination = $tradeQry->paginate($pagination_qty);
        // $memotradeData=$tradeQry->get();
        // dd($memotradeData);

        return view("backend.memo.trade.index", compact('memotradeData', 'pagination_qty', 'sort_search'));
    }
    ...
    public function return(Request $request)
    {
      ...
      Memo::select('memos.id as memoid','memo_details.id as memodetailsid','memo_details.item_status','retail_resellers.*',
                              ...
                              ->orderBy('memo_details.status_updated_date', 'DESC')
                              ->groupBy('memo_details.memo_id');
                            //   ->orderBy('memo_details.created_at,'memo_id')
        ...
        if($sort_search != null){
         ...
         {
            $memoreturnData = $returnQry->paginate($pagination_qty);
         }
        // $pagination = $returnQry->paginate($pagination_qty);
        // $memoreturnData=$returnQry->get();
// dd($memoreturnData);

        return view("backend.memo.return.index", compact('memoreturnData','pagination_qty', 'sort_search'));
    }
    ...
    public function trade_ngd(Request $request)
    {
       ...
       $tradeQry =Memo::select('memos.id as memoid','memo_details.id as memodetailsid','memo_details.item_status','retail_resellers.*',
        ...
        if( $request->pagination_qty == "all")
         ...
         {
            $memotrade_ngdData = $tradeQry->paginate($pagination_qty);
         }
        // $pagination = $tradeQry->paginate($pagination_qty);
        // $memotrade_ngdData=$tradeQry->get();
        // dd($memotrade_ngdData);

        return view("backend.memo.trade-ngd.index", compact('memotrade_ngdData', 'pagination_qty','sort_search'));
    }
