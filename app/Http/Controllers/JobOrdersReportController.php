...
// dd($complete_report);
        return view("backend.jobOrder_report.complete_report", compact('sort_search','complete_report','pagination_qty','startrangedate','endrangedate'));
    }
    public function pending_job_report(Request $request)
    {
        ...
        // dd($startdate.' 00:00:00');
      if ($request->startrangedate || $request->endrangedate) {
          $pending_job = $pending_job->whereBetween('job_orders.created_at', [$startdate.' 00:00:00',$endate.' 23:59:59']);
      } 
        $sort_search = isset($request->search) ? $request->search : '';
        if($sort_search != null){
        ...
        return view("backend.jobOrder_report.pending_job_report", compact('sort_search','pending_job_report','pagination_qty','startrangedate','endrangedate'));
    }
    public function OnHoldJob_report(Request $request)
    {
      ...
      if ($request->startrangedate || $request->endrangedate) {
        ...
        // $OnHoldJob_report = $OnHoldJob->paginate($pagination_qty);
        return view("backend.jobOrder_report.OnHoldJob_report", compact('sort_search','OnHoldJob_report','pagination_qty','startrangedate','endrangedate'));
    }
    public function OpenJob_report(Request $request)
    {
      ...
      if ($request->startrangedate || $request->endrangedate) {
        ...
        return view("backend.jobOrder_report.OpenJob_report", compact('sort_search','OpenJob_report','pagination_qty','startrangedate','endrangedate'));
    }
    public function PastDueJob_report(Request $request)
    {
      ...
      if ($request->startrangedate || $request->endrangedate) {
          ...
          if( $request->pagination_qty == "all"){
            $PastDueJob_report = $PastDueJob->get();
          }else{
            $PastDueJob_report = $PastDueJob->paginate($pagination_qty);
          }
        return view("backend.jobOrder_report.PastDueJob_report", compact('sort_search','PastDueJob_report','pagination_qty','startrangedate','endrangedate'));
    }
    ...
    //   return Excel::download($fetchLiaat, 'Agent_report.xlsx');
                  ...
                  $fetchLiaat = new AgentReportExcel($proID);
                  $dt = new \DateTime();
                  $curntDate = $dt->format('m-d-Y');
                  return Excel::download($fetchLiaat, 'Agent_report'.$curntDate.'.xlsx');
    }
    ...
    //   return Excel::download($fetchLiaat, 'Client_report.xlsx');
                  ...
                  // dd($ids);
                  $proID = json_decode($ids, TRUE);
                  $fetchLiaat = new ClientReportExcel($proID);
                  $dt = new \DateTime();
                  $curntDate = $dt->format('m-d-Y');
                  return Excel::download($fetchLiaat, 'ClientReportExcel'.$curntDate.'.xlsx');
    }
    public function complete_report_export(Request $request)
    {
        ...
        // return Excel::download($fetchLiaat, 'Complete_report.xlsx');
                  ...
                  // dd($ids);
                  $proID = json_decode($ids, TRUE);
                  $fetchLiaat = new CompleteReportExcel($proID);
                  $dt = new \DateTime();
                  $curntDate = $dt->format('m-d-Y');
                  return Excel::download($fetchLiaat, 'CompleteReportExcel'.$curntDate.'.xlsx');
    }
    public function pendingJob_report_export(Request $request)
    {
        ...
        // return Excel::download($fetchLiaat, 'pendingJob_report.xlsx');
                  ...
                  // dd($ids);
                  $proID = json_decode($ids, TRUE);
                  $fetchLiaat = new PendingJobReportExcel($proID);
                  $dt = new \DateTime();
                  $curntDate = $dt->format('m-d-Y');
                  return Excel::download($fetchLiaat, 'PendingJobReportExcel'.$curntDate.'.xlsx');
    }
    public function onHoldJob_report_export(Request $request)
    {
        ...
        // return Excel::download($fetchLiaat, 'onHoldJob_report_export.xlsx');
                  ...
                  // dd($ids);
                  $proID = json_decode($ids, TRUE);
                  $fetchLiaat = new OnHoldJobReportExcel($proID);
                  $dt = new \DateTime();
                  $curntDate = $dt->format('m-d-Y');
                  return Excel::download($fetchLiaat, 'OnHoldJobReportExcel'.$curntDate.'.xlsx');
    }
    public function openJob_report_export(Request $request)
    {
        ...
        // return Excel::download($fetchLiaat, 'openJob_report_export.xlsx');
                  ...
                  // dd($ids);
                  $proID = json_decode($ids, TRUE);
                  $fetchLiaat = new OpenJobReportExport($proID);
                  $dt = new \DateTime();
                  $curntDate = $dt->format('m-d-Y');
                  return Excel::download($fetchLiaat, 'OpenJobReportExport'.$curntDate.'.xlsx');
    }
    public function pastDueJob_report_export(Request $request)
    {
        ...
        // return Excel::download($fetchLiaat, 'pastDueJob_report_export.xlsx');
                  ...
                  // dd($ids);
                  $proID = json_decode($ids, TRUE);
                  $fetchLiaat = new PastDueJobExcel($proID);
                  $dt = new \DateTime();
                  $curntDate = $dt->format('m-d-Y');
                  return Excel::download($fetchLiaat, 'PastDueJobExcel'.$curntDate.'.xlsx');
    }
}
?>