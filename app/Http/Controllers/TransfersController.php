...
class TransfersController extends Controller
{
   // Return Start
    public function transfer(Request $request)
    {
      ...
      //   }
        $sort_search = null;
      $PurchasesProduct = Transfer::select('transfers.*','transfer_items.product_id','transfer_items.transfer_id',DB::raw('SUM(transfer_items.quantity) AS quantity'),'products.model')
      ...
      if( $request->pagination_qty == "all")
         ...
         {
            $transfer = $PurchasesProduct->paginate($pagination_qty);
         }
      // DB::raw("(GROUP_CONCAT(transfer_items.product_code SEPARATOR ',')) as `product_code`")
      // dd($transfer);
        return view("backend.product.transfers.index", compact('transfer','sort_search','pagination_qty'));
    }
    ...
    //   dd($proidmailArr);
        ...
        $product=Product::select('products.*','product_stocks.sku','product_stocks.qty')
        ->leftJoin('product_stocks','products.id','=','product_stocks.product_id')
        ->whereIn('products.id',$proidmailArr)
        // ->leftJoin('transfer_items', 'transfer_items.product_id', '=', 'products.id')
        ->get();
        // dd($product);
    //   
      ...
      $post->save();
       ...
       $ReturnProData = Transfer::where('transfers.id',$transfer_id)
...
// dd($ReturnProData);
        ...
        $array['product_data']=$product;
        $array['ReturnProData']=$ReturnProData;
        $array['user']=$curr_uid;
        $array['stock_id'] =  $returnProData->stock_id;
        $array['from_war'] =$wfName;
        $array['to_ware'] = $wtName;
        $array['unit_price']=$subtotalReturn;
        $array['subtotal']=$subtotalReturn;
        $pdf = PDF::loadView('backend.product.transfers.mailpdf', $array);
        $array['from'] = env('MAIL_FROM_ADDRESS');
        Mail::send('frontend.backendMailer.add_transfer', $array, function($message) use ($array,$pdf) {
            $message->to(env("StockManager"));
            $message->subject('User Activity Notification');
            $message->attachData($pdf->output(), "transfer.pdf");
        });
    ...
    public function transferSearchAjax(Request $request)
    {
     ...
     $response = array();
    //  $autocomplete=array_reverse($autocomplete);
     foreach($autocomplete as $autoData){
       if(!empty($autoCompId) && in_array($autoData->id,$autoCompId)){
         continue;
       }
    ...
    public function preReturn(Request $request)
    {
      ...
      if($returnID != "")
      {
        ...
        foreach ($ReturnItemData as $reKey => $RProItem) {
          $reKeyCt =$reKey + 1;
