...
class ReturnController extends Controller
{
   // Return Start
    public function return(Request $request)
    {
      ...
      $sort_search = null;
       $PurchasesProduct = ReturnItems::select('return_items.*','return_prods.id as ret_id','return_prods.reference_no','return_prods.return_date','return_prods.supplier_id','products.name','products.stock_id','products.model','products.weight','products.paper_cart','products.custom_1','products.custom_2','products.custom_3','products.custom_4','products.custom_5','products.custom_6','products.custom_7','products.custom_8','products.custom_9','products.custom_10','sellers.company as supplier_name')
       ->join('return_prods','return_prods.id','=','return_items.return_id')
       ->join('sellers','sellers.user_id','=','return_prods.supplier_id')
        ...
        if(!empty($reaArrData)){
          ...
          $subtotalReturn = $returnProData->product_cost* $proQty;
            ...
            $curr_name = $user->name;
              ...
              $proLog->activity = addslashes('STOCK ID <a href="">'.$sotck_id.'</a> is returned to <a href="#">'. $supplier_name.'</a> on');
      ...
      if(!empty($reaArrData)){
        ...
        $subtotalReturn = $returnProData->product_cost* $proQty;
          ...
          $curr_name = $user->name;
            ...
            $proLog->user_id = $curr_uid;
            $proLog->prodcut_id = $proid;
            $proLog->activity = addslashes('STOCK ID <a href="">'.$sotck_id.'</a> is returned to '. $supplier_name.'</a> on');
            $proLog->action = 'movedToReturns';
            $proLog->save();
    ...
    {
    $getproID = ReturnItems::where('return_id',$id)->first();
        $proID = $getproID->product_id;
        $pro_qty = ProductStock::where('product_id',$proID)->first();
        $pro_qty->qty += 1;
        $pro_qty->save();
        ...
        $user = Auth::user();
            ...
            $curr_name = $user->name;
              ...
              $proLog->user_id = $curr_uid;
              $proLog->prodcut_id = $proID;
              $proLog->activity = addslashes(' Item is reverted back to inventory on');
              $proLog->action = 'movedToReturns';
              $proLog->save();
