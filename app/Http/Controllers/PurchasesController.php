   ...
   // Warehouse Start
    public function purchases(Request $request)
    {
      $sort_search =null;
        ...
        $PurchasesProduct=Product::with('reviews', 'brand', 'stocks', 'user', 'user.shop','productcondition','productType')
        ...
        ->groupBy('products.id')
        ->select('products.*','warehouse.name as warehouseName','users.name as supplienName','memo_details.product_id','memo_details.item_status','product_types.listing_type','brands.name as brandName','product_stocks.sku','product_stocks.qty','productconditions.name as productconditions_name')
      ...
      if ($request->startrangedate || $request->endrangedate) {
          $PurchasesProduct =  $PurchasesProduct->whereBetween('products.dop', [$startdate.' 00:00:00',$endate.' 23:59:59']);
      }
       ...
       $product_qty=$request->purchases_pagi;
        ...
        return view("backend.purchases.index", compact('PurchasesData','pagination_qty','sort_search','startrangedate','endrangedate'));
    }
