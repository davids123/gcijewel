...

{
    ...
    public function appraisal( Request $request)
    {
        ...
        {
           $data= $data->orWhere('template_name', 'like', '%'.$request->search.'%');
          $data=  $data->orWhere('stock_id', 'like', '%'.$request->search.'%');
          $data=  $data->orWhere('listing_type', 'like', '%'.$request->search.'%');
           $data=  $data->orWhere('manufacturer', 'like', '%'.$request->search.'%');
            $data= $data->orWhere('model_name', 'like', '%'.$request->search.'%');
            $data= $data->orWhere('factory_model', 'like', '%'.$request->search.'%');
            $data= $data->orWhere('serial_no', 'like', '%'.$request->search.'%');
            $data= $data->orWhere('sp_value', 'like', '%'.$request->search.'%');
            $data= $data->orWhere('manufacturer', 'like', '%'.$request->search.'%');
    ...
    public function StockIdAjax(Request $request)
...

    {
      ...
      // $apparisalVal=Appraisal::where('id',$id)->select('id','stock_id','image')->first();
      // dd($apparisalVal)
      // echo ;
      $stock_id=$apparisalVal->stock_id;
      $img_path=uploaded_asset($apparisalVal->image);
      $img="<p><b>image</b></p><img src='".$img_path."' width='100px' alt='".$stock_id."'>";
