...
class ShortStockExcel implements FromCollection, WithMapping, WithHeadings
{
    ...
    {  
        ...
        $Shortstock = MemoDetail::select('memo_details.id','products.model','memo_details.product_qty',DB::raw('sum(product_stocks.qty) as stockqtysum'),'memo_details.item_status',DB::raw('sum(memo_details.product_qty) as  memoqtysum'))
                    ->leftJoin('products','products.id','=','memo_details.product_id')
                    ->leftJoin('product_stocks','products.id','=','product_stocks.product_id')
                    // ->leftjoin('memo_details', 'memo_details.memo_id', '=', 'memos.id')
